...
public class AggregationFunctionFactory {
  ...
  public static AggregationFunction getAggregationFunction(FunctionContext function, boolean nullHandlingEnabled) {
    try {
      ...
      if (upperCaseFunctionName.startsWith("PERCENTILE")) {
        return getPercentileAggregationFunction(function, upperCaseFunctionName, arguments, numArguments,
            nullHandlingEnabled);
      } else {
        return getOtherAggregationFunction(upperCaseFunctionName, arguments, numArguments, nullHandlingEnabled);
      }
    } catch (Exception e) {
  ...
  private static AggregationFunction getPercentileAggregationFunction(FunctionContext function,
      String upperCaseFunctionName, List<ExpressionContext> arguments, int numArguments,
      boolean nullHandlingEnabled) {
    String remainingFunctionName = upperCaseFunctionName.substring(10);
    ExpressionContext firstArgument = arguments.get(0);

    switch (remainingFunctionName) {
      case "SMARTTDIGEST":
      ...
      default:
        return getPercentileAggregationFunctionByNumArguments(function, remainingFunctionName, arguments,
            numArguments, firstArgument, nullHandlingEnabled);
    }
  }

  private static AggregationFunction getPercentileAggregationFunctionByNumArguments(
      FunctionContext function, String remainingFunctionName, List<ExpressionContext> arguments,
      int numArguments, ExpressionContext firstArgument, boolean nullHandlingEnabled) {
    if (numArguments == 1) {
      return getSingleArgumentPercentileFunction(function, remainingFunctionName, firstArgument,
          nullHandlingEnabled);
      ...
      return getDoubleArgumentPercentileFunction(remainingFunctionName, firstArgument, percentile,
          nullHandlingEnabled);
      ...
      return getTripleArgumentPercentileFunction(remainingFunctionName, firstArgument, percentile,
          compressionFactor, nullHandlingEnabled);
    } else {
      throw new IllegalArgumentException("Invalid percentile function: " + function);
    }
  }

  private static AggregationFunction getSingleArgumentPercentileFunction(FunctionContext function,
      String remainingFunctionName, ExpressionContext firstArgument, boolean nullHandlingEnabled) {
    ...
    if (remainingFunctionName.matches("\\d+")) {
      ...
      return new PercentileRawTDigestMVAggregationFunction(firstArgument,
          parsePercentileToInt(percentileString));
    } else {
      throw new IllegalArgumentException("Invalid percentile function: " + function);
    }
  ...
  private static AggregationFunction getDoubleArgumentPercentileFunction(String remainingFunctionName,
      ExpressionContext firstArgument, double percentile, boolean nullHandlingEnabled) {
    ...
    switch (remainingFunctionName) {
      ...
      default:
        throw new IllegalArgumentException("Invalid percentile function: " + remainingFunctionName);
    }
  }

  private static AggregationFunction getTripleArgumentPercentileFunction(String remainingFunctionName,
      ExpressionContext firstArgument, double percentile, int compressionFactor,
      boolean nullHandlingEnabled) {
    // Triple arguments percentile (e.g. percentileTDigest(bar, 95, 1000), etc.) where the
    // second argument is a decimal number from 0.0 to 100.0 and third argument is a decimal
    // number indicating
    // the compression_factor for the TDigest. This can only be used for TDigest type percentile
    // functions to
    // pass in a custom compression_factor. If the two argument version is used the default
    // compression_factor
    // of 100.0 is used.
    switch (remainingFunctionName) {
      case "TDIGEST":
      ...
      default:
        throw new IllegalArgumentException(
            "Invalid percentile function with three arguments: " + remainingFunctionName);
    }
  }

  private static AggregationFunction getOtherAggregationFunction(String upperCaseFunctionName,
      List<ExpressionContext> arguments, int numArguments, boolean nullHandlingEnabled) {
    ...
    switch (functionType) {
      ...
      case FIRSTWITHTIME:
        return getFirstWithTimeAggregationFunction(arguments, numArguments, nullHandlingEnabled);
      case LISTAGG:
        return getListAggFunction(arguments, numArguments, nullHandlingEnabled);
      case ARRAYAGG:
        return getArrayAggFunction(arguments, numArguments, nullHandlingEnabled);
      case LASTWITHTIME:
        return getLastWithTimeAggregationFunction(arguments, numArguments, nullHandlingEnabled);
      case MINMAXRANGE:
  ...
  private static AggregationFunction getFirstWithTimeAggregationFunction(List<ExpressionContext> arguments,
      int numArguments, boolean nullHandlingEnabled) {
  ...
  private static AggregationFunction getListAggFunction(List<ExpressionContext> arguments, int numArguments,
      boolean nullHandlingEnabled) {
  ...
  private static AggregationFunction getArrayAggFunction(List<ExpressionContext> arguments, int numArguments,
      boolean nullHandlingEnabled) {
    ...
    if (isDistinct) {
      return getDistinctArrayAggFunction(firstArgument, dataType, nullHandlingEnabled);
    } else {
      return getArrayAggFunction(firstArgument, dataType, nullHandlingEnabled);
    }
  }

  private static AggregationFunction getDistinctArrayAggFunction(ExpressionContext firstArgument,
      DataType dataType, boolean nullHandlingEnabled) {
    switch (dataType) {
      case BOOLEAN:
      case INT:
  ...
  private static AggregationFunction getArrayAggFunction(ExpressionContext firstArgument, DataType dataType,
      boolean nullHandlingEnabled) {
    switch (dataType) {
      case BOOLEAN:
      case INT:
  ...
  private static AggregationFunction getLastWithTimeAggregationFunction(List<ExpressionContext> arguments,
      int numArguments, boolean nullHandlingEnabled) {
    Preconditions.checkArgument(numArguments == 3,
